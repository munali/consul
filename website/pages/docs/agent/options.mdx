...
- `service_health_streaming_backend` - When enabled Consul client agents will use
  streaming rpc to receive updates about service health, instead of the traditional
  blocking queries. All servers must have `rpc.enable_streaming` enabled before any
  client can use `service_health_streaming_backend`. At least one of `dns.use_cache`
  or `http_config.use_cache` must be enabled, otherwise this setting has no effect.
